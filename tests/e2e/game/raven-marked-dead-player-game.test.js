    ...
    it("🪶 Raven marks the villager (POST /games/:id/play)", done => {
        ...
        chai.request(app)
            ...
            .end((err, res) => {
                expect(res).to.have.status(200);
                game = res.body;
                expect(game.players[1].attributes).to.deep.include({ attribute: "raven-marked", source: "raven", remainingPhases: 2 });
                expect(game.history[0].play.targets).to.exist;
    ...
    it("👪 One vote only for raven, dead villager is not nominated despite the fact he has two votes (POST /games/:id/play)", done => {
        ...
        chai.request(app)
            ...
            .end((err, res) => {
                ...
                expect(game.players[1].attributes).to.not.deep.include({ attribute: "raven-marked", source: "raven", remainingPhases: 2 });
                done();
            });
    });
});